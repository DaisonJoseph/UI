...
$result = $collection->find(array("Dead_Jobs"=>"Yes"))->count();
echo "Dead Jobs Count"." ".$result."<br><br>";

//total no. of component types
$types = $collection->distinct("Component_Type");
$component_type_arr = array();
foreach($types as $type){
	$value = $collection->find(array("Component_Type"=>$type,"Dead_Jobs"=>"Yes"))->count();
...
foreach($types as  $type)
   {
		$comp_total_loc = $collection->aggregate(array('$match'=>array("Component_Type"=>$type,"Dead_Jobs"=>"Yes")),array('$group'=>array('_id'=>'null','TotalLLoc'=>array('$sum'=>'$LLOC'))));
...
foreach($types as  $type)
   {
		$comp_total_loc = $collection->aggregate(array('$match'=>array("Component_Type"=>$type,"Dead_Jobs"=>"Yes")),array('$group'=>array('_id'=>'null','TotalLLoc'=>array('$sum'=>'$ULOC'))));
		foreach($comp_total_loc as $c)
		{
			foreach($c as $c1)
			{
