...
foreach($types as  $type)
   {
		$comp_total_loc = $collection->aggregate(array('$match'=>array("Component_Type"=>$type)),array('$group'=>array('_id'=>'null','TotalLLoc'=>array('$sum'=>'$LLOC'))));
...
foreach($types as  $type)
   {
		$comp_total_loc = $collection->aggregate(array('$match'=>array("Component_Type"=>$type)),array('$group'=>array('_id'=>'null','TotalLLoc'=>array('$sum'=>'$ULOC'))));
		foreach($comp_total_loc as $c)
		{
			foreach($c as $c1)
			{
...
foreach($types as  $type)
   {
		$comp_total_loc = $collection->aggregate(array('$match'=>array("Component_Type"=>$type)),array('$group'=>array('_id'=>'null','TotalLLoc'=>array('$sum'=>'$CLOC'))));
		foreach($comp_total_loc as $c)
		{
			foreach($c as $c1)
			{
