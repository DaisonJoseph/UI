...
function createContainer2(container, seriesType) {
  ...
  <?php
	...
	{
		$type = $distinct_component;
		$value = $MI_col -> find(array("Component_Type"=>$distinct_component))->count();
		$component_array[$type][] = $value;
		$comp_total_lloc = $MI_col->aggregate(array('$match'=>array("Component_Type"=>$distinct_component)),array('$group'=>array('_id'=>'null','TotalLLoc'=>array('$sum'=>'$LLOC'))));
		foreach($comp_total_lloc as $c)
		{
			foreach($c as $c1)
				{
					$component_array[$type][] = $c1['TotalLLoc'];
				}	
		}
		$comp_total_uloc = $MI_col->aggregate(array('$match'=>array("Component_Type"=>$distinct_component)),array('$group'=>array('_id'=>'null','TotalLLoc'=>array('$sum'=>'$ULOC'))));
		foreach($comp_total_uloc as $c)
  ...
  chart.data = [
	...
	{
    "type": "<?php echo $key; ?>",
	"litres":<?php echo $value[0];?>,
	"subData": [{ name: "LLOC", value: <?php echo $value[1] ;?> }, { name: "ULOC", value: <?php echo $value[2] ?> }, { name: "CLOC", value: <?php echo $value[3] ?> }]
  },
  <?php
		}
  ?>
