                  ...
                  <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12">
                      <div class="x_panel container drop-shadow" style="background-color: white;">
                        <div class="x_title">
                          <div class="col-md-12 col-sm-12 col-xs-12">
                            <h2>Load Vs Source</h2>
							<?php
								if (isset($_GET['input']))
								{	
									$business =$_GET['input'];
									$business = str_replace(" ","+",$business);
									echo '<h5><a href="loadVsSourceReportdownload.php?input='. $business .'"> Download Excel Report </a></h5>';
								}
								else
								{
									echo '<h5><a href="loadVsSourceReportdownload.php"> Download Excel Report </a></h5>';
								}
								?>
                            <form action="loadVsSourceReport.php" class="searchform">
                          ...
                          <div class="table-responsive">
                            <table id="example" class="table table-bordered drop-shadow" style="width:100%">
							  ...
							  if (isset($_GET['input']) || isset($_GET['search'])) {
                                  ...
                                  $Loads = array('Load' => $input);
								  $Comp = array('Component' => $input);

								   $res1 = 0;
								   $res2 = 0;
								   $res1 = $collection->find($Loads)->count();
								   $res2 = $collection->find($Comp)->count();
								   #echo $res1;
								   echo $res2;
								   
								   if ($res1 > 0)
								   {
									   $srch1 = $collection->find($Loads)->limit(500);
										echo "inisde";
										foreach ($srch1 as $srch) {
										$component = $srch['Load'];
										$string = $srch['Load Available, Source NA'];
										echo '<tr>';
								   ...
								   if ($res2 > 0)
								   {
									   $srch1 = $collection->find($Comp)->limit(500);
									   
										foreach ($srch1 as $srch) {
										$component = $srch['Component'];
										$string = $srch['Source Available,Load NA'];
										echo '<tr>';
										echo '<td>' . $component . '</td>';
										echo '<td>' . $string . '</td>';
										echo '</tr>';
								...
								$results = $collection->find(array("Source Available,Load NA" => "Load Not Available"))->limit(500)->skip($from);
                                #$results1 = $collection->find(array("Load Available, Source NA" => "Source Not Available"))->limit(500)->skip($from);
								...
								$results = $collection->find(array("Load Available, Source NA" => "Source Not Available"))->limit(500)->skip($from);
                                

                                foreach ($results as $result) {
                                  $component = $result['Load'];
                                  $string = $result['Load Available, Source NA'];
									if ($component !== ""){
									  echo '<tr>';
									  echo '<td>' . $component . '</td>';
									  echo '<td>' . $string . '</td>';
