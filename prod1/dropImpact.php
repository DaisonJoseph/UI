...
$total = $collection->find(array("drop_impact" => "Yes"))->count();
                  ...
                  <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12">
                      <div class="x_panel container drop-shadow" style="background-color: white;">
                        <div class="x_title">
                          <div class="col-md-12 col-sm-12 col-xs-12">
                            <h2>Drop Impact Report</h2>
                            <?php
                              if (isset($_GET['input'])) {
                                $business = $_GET['input'];
                                $business = str_replace(" ", "+", $business);
                                echo '<h5><a href="dropImpactReportsdownload.php?input=' . $business . '"> Download Excel Report </a></h5>';
                              } else {
                                echo '<h5><a href="dropImpactReportsdownload.php"> Download Excel Report </a></h5>';
                              }
                              ?>
                            <form action="dropImpact.php" class="searchform">
                          ...
                          <div class="table-responsive">
                            <table id="example" class="table table-bordered drop-shadow" style="width:100%">
                              ...
                              <?php
                                if (isset($_GET['input']) || isset($_GET['search'])) {
                                  ...
                                  if ($numdocs == 0) {
                                    $input = str_replace(" ", "-", $input);
                                    $results = $collection->find($inputs)->sort(array("_id" => 1));
                                  }
                                } else {
                                  $results = $collection->find(array('drop_impact' => 'Yes'))->sort(array("component_type" => 1))->limit(500)->skip($from);
                                }
                                echo '<tbody>';
							...
							<div class="row">
                              ...
                              if(isset($previousFrom)){
                                ?>
                                <a href="dropImpact.php?from=<?php echo $previousFrom; ?>&to=<?php echo $previousTo; ?>" class="btn btn-primary"><?php echo $previousFrom." - ".$previousTo; ?></a>
                                <?php
                              }
                              ?>
                              </div>
                              ...
                              if(isset($nextFrom)){
                                ?>
                                <a href="dropImpact.php?from=<?php echo $nextFrom; ?>&to=<?php echo $nextTo; ?>" class="btn btn-primary pull-right"><?php echo $nextFrom." - ".$nextTo; ?></a>
                                <?php
                              }
                              ?>
                              </div>
